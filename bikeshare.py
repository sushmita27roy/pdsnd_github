    ...
    while True:
        ...
        try:
            if input_read in ['chicago','new york city','washington'] and input_type == 1:
                break
            elif input_read in ['jan','feb','mar','apr','may','jun','all'] and input_type == 2:
                break
            elif input_read in ['mon','tue','wed','thu','fri','sat','sun','all'] and input_type == 3:
                break
            else:
                if input_type == 1:
                    print("Sorry, your Input should be: chicago, new york city or washington")
                if input_type == 2:
                    print("Sorry, your Input should be: jan, feb, mar, apr, may, jun or all")
                if input_type == 3:
                    print("Sorry, your Input should be: mon, tue, wed, thu, fri, sat, sun or all")
        except ValueError:
            print("Sorry, that's a wrong Input")
        except KeyboardInterrupt:
            print("\nInvalid Input")
        except IndexError:
            print("\nOops! Please Select another day.")
            break
        finally:
            pass
    return input_read
...
def get_filters():
    ...
    month = check_input("Which Month? (jan, feb, mar, apr, may, jun or all)", 2)
    # get user input for day of week (all, monday, tuesday, ... sunday)
    day = check_input("Which day? (mon, tue, wed, thu, fri, sat, sun or all)", 3)
    print('-'*40)
...
def load_data(city, month, day):
    ...
    print("You have selected to view the data For :\nCity:",city.title())
...
def output_display5(df):
    
    display=0
    while True:
        choice=input('Would you to like to view 5 lines of the Raw Data? Enter: yes or no: ')
        if choice.lower() == 'yes':
